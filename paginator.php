...
if($page_current > 0){
	if($a != 0 && $b != 0 && $c != 0 && $page_current > 0){
		if(is_numeric($a) && is_numeric($b) && is_numeric($c)){
			if((int)$a == floatval($a) && (int)$b == floatval($b) && (int)$c == floatval($c)){
				if($a > $b){
					...
					echo '<b style = "color: blue">Không có bản ghi nào !!</b>';
				}
			}else{
		...
		if(isset($_POST['submit'])){
			echo '<b style = "color: red">Bạn chưa nhập giá trị! Vui lòng nhập giá trị vào !</b>';
		}
	}	
}else{
	echo '<b style = "color: red">Trang không tồn tại !</b>';
}
